...
class ProgramGenerationModel(LightningModule):
    ...
    def validation_epoch_end(self, outputs: List[Dict[str, Any]]) -> None:
        ...
        program_generation_output_dir = "program_generation_output_new"
        if not os.path.exists(program_generation_output_dir):
            os.mkdir(program_generation_output_dir)

        output_prediction_file = os.path.join(program_generation_output_dir,  "dev_predictions.json")
        output_nbest_file = os.path.join(program_generation_output_dir,"dev_nbest_predictions.json")
        output_eval_file = os.path.join(program_generation_output_dir, "dev_full_results.json")
        output_error_file = os.path.join(program_generation_output_dir, "dev_full_results_error.json")
        
        with open(valid_file) as input_file:
            input_data = json.load(input_file)

        data_ori = []
        for entry in input_data:
            example = read_mathqa_entry(entry, self.tokenizer)
            if example:
        ...
        exe_acc = evaluate_result(
            output_nbest_file, valid_file, output_eval_file, output_error_file, program_mode="seq")

        print(f"exe_acc: {exe_acc}")
