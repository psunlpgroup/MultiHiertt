...
class FinQADataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        train_data = FinQADataset(file_path=self.train_file_path,
                                   max_instances=self.train_max_instances, 
                                   mode="train")
        self.train_data = train_data

        val_data = FinQADataset(file_path=self.val_file_path,
                                 max_instances=self.val_max_instances, 
                                 mode="valid")
        self.val_data = val_data 
        
        test_data = FinQADataset(file_path=self.test_file_path,
                                 max_instances=self.val_max_instances, 
                                 mode="test")
    ...
    def test_dataloader(self):
        ...
        dtloader = DataLoader(self.test_data, batch_size=self.val_batch_size, shuffle=True, drop_last=False, collate_fn=customized_collate_fn)
        return dtloader